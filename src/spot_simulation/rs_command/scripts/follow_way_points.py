...
class WpsService:
    def __init__(self):
        self.get_wps_serivce = rospy.Service('/get_way_points', ReqWayPoints, self.GetWayPoints)
        self.wps = ReqWayPointsRequest()
        self.bool_msg = Bool(False)
    
    def GetWayPoints(self,req):
        self.wps = req.points
        self.wps.reverse()
        #for wp in wps:
        #    print(wp.x,wp.y,wp.z)
        self.bool_msg = Bool(True)
        return ReqWayPointsResponse(self.bool_msg)
...
if __name__ == "__main__":
    ...
    ##spwan the the model of the way points
    # wp_pose = Pose()
    # wp_pose.position.x = 0
    # wp_pose.position.y = 0
    # wp_pose.position.z = 0

    # file_location = "/mnt/shared/spot_sim_ws/src/spot_simulation/rs_command/urdf/marker.urdf.xacro"
    # ps = os.popen("rosrun xacro xacro" + file_location)
    # xml_string = ps.read()
    # ps.close()
    # spawn_model_prox = rospy.ServiceProxy('/gazebo/spawn_urdf_model', SpawnModel)
    # req = SpawnModelRequest()
    # req.model_name = goal_marker_name
    # req.model_xml = xml_string
    # req.initial_pose = wp_pose

    # res = spawn_model_prox(req)
    ...
    ws = WpsService()
    #wait until the service is called then we get the path
    while not ws.bool_msg.data:
        rate.sleep()
    ...
    try:
        ...
        for wp in ws.wps:
            ...
            #skip some way points, or there will be too many way points
            skip += 1
            if skip%3 != 0:
                continue

            print("current goal ",goal_x,goal_y)

            ##spwan the the model of the way points. For every way point I try to generate a way point, but it seems this will make the spot model very unstable. I dont know why yet
            # wp_pose = Pose()
            # wp_pose.position.x = goal_x
            # wp_pose.position.y = goal_y
            # wp_pose.position.z = 0
            ...
            # # evaluate response
            # if res.success == True:
            #     rospy.loginfo(res.status_message + " " + goal_marker_name)
            # else:
            #     print("Error: model %s not spawn. error message = "% goal_marker_name + res.status_message)
...
#         print(wp.x,wp.y,wp.z)
#     bool_msg = Bool(True)
#     return ReqWayPointsResponse(bool_msg)

# def FollowWayPointsServer():
#     rospy.init_node('follow_way_points_server')
#     s = rospy.Service('get_way_points', ReqWayPoints, GetWayPoints)
#     print("got service")
#     #rospy.spin()

# if __name__ == "__main__":
#     FollowWayPointsServer()